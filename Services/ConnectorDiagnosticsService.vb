    ...
    Public Class ConnectorDiagnosticsService
            ...
            If rows.Count = 0 Then
                ...
                For Each it In items
                    ...
                    For Each ro In refs
                        ...
                        If ElementIdCompat.IntValue(rc.Owner.Id) = it.OwnerId Then Continue For

                        Dim rcOwnerId = ElementIdCompat.IntValue(rc.Owner.Id)
        ...
        Private Shared Function BuildRow(e1 As Element, e2 As Element, distInch As Double, a As Connector, b As Connector, param As String, Optional forceConnected As Boolean = False) As Dictionary(Of String, Object)
            ...
            Return New Dictionary(Of String, Object)(StringComparer.Ordinal) From {
                {"Id1", ElementIdCompat.IntValue(e1.Id).ToString()},
                {"Id2", ElementIdCompat.IntValue(e2.Id).ToString()},
                {"Category1", cat1},
                {"Category2", cat2},
                {"Family1", fam1},
                {"Family2", fam2},
            ...
            Try
                For Each e In New FilteredElementCollector(doc).OfClass(GetType(MEPCurve)).WhereElementIsNotElementType().ToElements()
                    ...
                    For Each o In cm.Connectors
                        ...
                        list.Add(New ConnItem With {.Owner = e, .OwnerId = ElementIdCompat.IntValue(e.Id), .Conn = c, .P = c.Origin, .IndexHint = idx})
                        idx += 1 : mep += 1
                    Next
                Next
                ...
                If a.IsConnected AndAlso b.IsConnected Then
                    ...
                    If refs IsNot Nothing AndAlso refs.Size > 0 Then
                        For Each ro In refs
                            ...
                            If rc.Owner IsNot Nothing AndAlso b.Owner IsNot Nothing AndAlso ElementIdCompat.IntValue(rc.Owner.Id) = ElementIdCompat.IntValue(b.Owner.Id) Then
                                Return True
                            End If
                        Next
                    End If
