...
#Region "핸들러"
        ...
        Private Sub HandleDuplicateDelete(app As UIApplication, payload As Object)
            ...
            For Each eid In eidList
                If doc.GetElement(eid) Is Nothing Then
                    ...
                    Dim row = System.Linq.Enumerable.FirstOrDefault(_lastRows, Function(r As DupRowDto) r.ElementId = Infrastructure.ElementIdCompat.IntValue(eid))
                    If row Is Not Nothing Then row.Deleted = True
        ...
        Private Sub HandleDuplicateRestore(app As UIApplication, payload As Object)
            ...
            For Each i In lastPack
                Dim r = System.Linq.Enumerable.FirstOrDefault(_lastRows, Function(x As DupRowDto) x.ElementId = i)
